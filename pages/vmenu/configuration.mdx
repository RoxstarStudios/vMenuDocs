...
| `vmenu_menu_toggle_key` | A valid [control ID](https://docs.fivem.net/game-references/controls/#controls). | Sets the control ID for the key that opens vMenu (keyboard/mouse only, controller key will always be (hold) ‘change view/select’). |
| `vmenu_noclip_toggle_key` | A valid [control ID](https://docs.fivem.net/game-references/controls/#controls).  | Sets the control ID for the noclip toggle key (keyboard/mouse only). |
...
| `vmenu_dynamic_weather_timer` | Any [integer](https://en.wikipedia.org/wiki/Integer) (whole number) | Sets the delay between dynamic weather changes in whole minutes. |
...
| `vmenu_default_time_hour` | Any [integer](https://en.wikipedia.org/wiki/Integer) (whole number) | The default in-game hour to set the time to whenever the resource is started (24 hours clock). |
| `vmenu_default_time_min` | Any [integer](https://en.wikipedia.org/wiki/Integer) (whole number) | The default in-game minute to set the time to whenever the resource is started. |
| `vmenu_ingame_minute_duration` | Any [integer](https://en.wikipedia.org/wiki/Integer) (whole number) | (Default: 2000) Sets the duration of 1 in-game minute to this value in real time (ms). Example: if this is set to 2000 (ms) then 1 in-game minute takes 2 seconds in real life. |
...
| `vmenu_teleport_to_wp_keybind_key` | A valid [control ID](https://docs.fivem.net/game-references/controls/#controls) | The control ID for the Teleport To Waypoint keybind. 168 / F7 by default. |
...
| `vmenu_enable_animals_spawn_menu` | `true` / `false` | Enables the animals menu in the **Player Appearance** » **Ped Spawner** menu. (May cause server crashes) |
